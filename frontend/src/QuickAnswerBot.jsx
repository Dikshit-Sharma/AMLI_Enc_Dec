...
const FAQ = [
  ...
  { q: 'What is AES-RSA?', a: 'AES/RSA is a hybrid scheme: a random dynamic AES key encrypts the body with AES/GCM, then that AES key is wrapped with RSA-OAEP using the public key, and the ciphertext is signed with SHA256withRSA. Output format: Base64(rsaKey:base64Cipher:signature).', keywords: ['rsa', 'aes-rsa', 'hybrid', 'oaep', 'key wrapping', 'signature', 'digital signature'] },
  { q: 'What key format should I use?', a: 'GCM requires a Base64-encoded 256-bit key. CBC accepts a raw string of 16, 24, or 32 characters (128/192/256-bit). AES/RSA requires a private key (PKCS#8) and public key (SPKI), provided as PRIVATE_KEY|PUBLIC_KEY separated by a pipe. Use the Key Generator to create keys in both formats.', keywords: ['key format', 'key length', 'base64 key', 'hex key', 'raw key', 'key size', 'rsa key', 'private key', 'public key'] },
  ...
  { q: 'What is the Cipher Tool?', a: 'The Cipher Tool provides AES-256 encryption and decryption in GCM, CBC, and RSA (hybrid AES-GCM + RSA key wrapping) modes. It runs entirely in your browser using the Web Crypto API — no data ever leaves your machine.', keywords: ['cipher tool', 'encrypt tool', 'decrypt tool', 'encryption page', 'crypto tool'] },
