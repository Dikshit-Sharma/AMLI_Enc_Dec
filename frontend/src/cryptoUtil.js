 ...
 * Accepts an SPKI-encoded (SubjectPublicKeyInfo) public key / public key
 * certificate content. OAEP uses SHA-1 + MGF1-SHA1 to match Java's
 * "RSA/ECB/OAEPPADDING" default.
 */
async function importRSAPublicKey(keyStr) {
  const raw = base64ToBytes(stripPEM(keyStr));
  return window.crypto.subtle.importKey(
    'spki', raw.buffer, { name: RSA_OAEP, hash: 'SHA-1' }, false, ['encrypt']
  );
}

/**
 * Import an RSA public key for signature verification (RSASSA-PKCS1-v1_5,
 * SHA-256 to match Java's "SHA256withRSA").
 */
async function importRSAVerifyKey(keyStr) {
  const raw = base64ToBytes(stripPEM(keyStr));
  return window.crypto.subtle.importKey(
    'spki', raw.buffer, { name: RSASSA_PKCS1, hash: 'SHA-256' }, false, ['verify']
  );
}
